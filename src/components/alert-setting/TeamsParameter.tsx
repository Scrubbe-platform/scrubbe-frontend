...
const TeamsParameter = ({ handleSave }: Props) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<TeamsParameterForm>({
    defaultValues: {
      webhookUrl: "",
      channelName: "",
    },
  ...
  const onSubmit = (data: TeamsParameterForm) => {
    // handle save
    handleSave(data);
    console.log(data);
  };
    ...
    <form className="flex flex-col gap-y-3" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Controller
        ...
        render={({ field }) => (
          <Input
            label="Webhook URL"
            placeholder="http://teams.microsoft.com/......"
