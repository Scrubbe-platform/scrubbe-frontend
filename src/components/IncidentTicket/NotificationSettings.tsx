...
const NotificationSettings = ({
  ...
  const notifications = notification.map((item, index) => ({
    id: item.incidentId ?? item.ticketId ?? `${item.kind ?? "notification"}-${index}`,
    title: item.title ?? "Incident notification",
    description: item.message ?? item.summary ?? "A new incident event was received.",
    createdAt: item.timestamp
      ? new Date(item.timestamp).toLocaleString()
      : "Just now",
    name: item.ticketId ?? item.priority ?? "Scrubbe",
  }));
        ...
        <div className="mt-5">
          {notifications.length === 0 ? (
            <div className="py-10 text-sm text-gray-500 dark:text-gray-400">
              Live incident notifications will appear here as the server emits them.
            </div>
          ) : (
