...
export const raiseIncidentSchema = z.object({
  ...
  service:        z.string().min(1, "Please select an affected service").refine((v) => v !== "select-service", "Please select a valid service"),
...
const severityToPriority: Record<RaiseIncidentFormValues["severity"], string> = {
  P0: "CRITICAL", P1: "CRITICAL", P2: "HIGH", P3: "MEDIUM", P4: "LOW",
};
...
const buildImpactSummary = (data: RaiseIncidentFormValues) =>
  [customerImpactLabels[data.customerImpact] ?? data.customerImpact, data.businessImpact ? `Business impact: ${data.businessImpact}` : "", data.recentChange ? `Recent change: ${data.recentChange}` : ""].filter(Boolean).join(". ");

// Severity — color per level (meaningful signal)
const severityConfig: Record<RaiseIncidentFormValues["severity"], { label: string; active: string }> = {
  P0: { label: "Full outage",  active: "border-red-500 bg-red-50 dark:bg-red-500/8 text-red-600 dark:text-red-400"          },
  P1: { label: "Critical",     active: "border-red-400 bg-red-50 dark:bg-red-500/8 text-red-500 dark:text-red-400"           },
  P2: { label: "Major",        active: "border-amber-500 bg-amber-50 dark:bg-amber-500/8 text-amber-600 dark:text-amber-400" },
  P3: { label: "Minor",        active: "border-yellow-400 bg-yellow-50 dark:bg-yellow-500/8 text-yellow-600 dark:text-yellow-400" },
  P4: { label: "Info",         active: "border-sky-400 bg-sky-50 dark:bg-sky-500/8 text-sky-600 dark:text-sky-400"           },
};
...
interface AttachedFile { id: string; file: File }

function EvidenceSection({ files, onAdd, onRemove }: { files: AttachedFile[]; onAdd: (f: File[]) => void; onRemove: (id: string) => void }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragging, setDragging] = useState(false);

  const handleFiles = (incoming: FileList | null) => {
    if (!incoming) return;
    const valid = Array.from(incoming).filter((f) => { if (f.size > MAX_FILE_SIZE) { toast.error(`${f.name} exceeds 25MB limit`); return false; } return true; });
...
const RaiseIncidentModal = ({ onClose }: { onClose?: () => void }) => {
  ...
  const { control, handleSubmit, watch, formState: { errors } } = useForm<RaiseIncidentFormValues>({
    ...
    defaultValues: { severity: "P2", environment: "Production", customerImpact: "unknown", title: "", description: "", firstNoticed: "alert-alarm-fired", recentChange: "", businessImpact: "", assignTo: "", notifyChannels: "sre-oncall-pagerduty", service: "select-service", warRoom: "not-required" },
  });

  const assigneeOptions = useMemo(() => [
    { value: "", label: "Select incident owner..." },
    ...members.map((m) => { const n = `${m.firstname ?? ""} ${m.lastname ?? ""}`.trim() || m.email; return { value: m.email, label: `${n} (${m.email})` }; }),
  ], [members]);
  ...
  const createMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message || "Unable to create the incident right now."),
  });

  const [watchedTitle, watchedEnv, watchedSeverity, watchedService] =
    [watch("title"), watch("environment"), watch("severity"), watch("service")];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-y-auto rounded-2xl border border-zinc-500 dark:border-zinc-700/60 bg-white dark:bg-zinc-950 shadow-xl">
        ...
        <form onSubmit={handleSubmit(async (data) => createMutation.mutateAsync(data))} className="space-y-3 p-6">

          {/* Core Details */}
          <FormSection title="Core Details">
            <div className="space-y-5">
              <Controller name="title" control={control} render={({ field }) => (
                <Input {...field} label="Incident Title *" placeholder="Brief description of what's failing" error={errors.title?.message} />
              )} />
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {(["P0","P1","P2","P3","P4"] as const).map((sev) => (
                    <Controller key={sev} name="severity" control={control} render={({ field }) => (
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span className="text-[9px] uppercase opacity-70 mt-0.5">{severityConfig[sev].label}</span>
                      </button>
                    )} />
                  ))}
                </div>
          ...
          <FormSection title="Context & Detail">
            <div className="space-y-5">
              <Controller name="description" control={control} render={({ field }) => (
                <TextArea {...field} label="What is happening? *" placeholder="Describe the symptoms, affected users, and what responders know so far." error={errors.description?.message} rows={4} />
              )} />
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <Controller name="firstNoticed" control={control} render={({ field }) => (
                  <Select {...field} label="What did you notice first? *" error={errors.firstNoticed?.message} options={[
                    { value: "alert-alarm-fired", label: "Alert or alarm fired" },
                    { value: "customer-report",   label: "Customer report"      },
                    { value: "latency-spike",      label: "Latency spike"        },
                  ]} />
                )} />
                <Controller name="recentChange" control={control} render={({ field }) => (
                  <Input {...field} label="Recent change? (optional)" placeholder="Recent deploy, config change, or rollback" />
                )} />
                <Controller name="customerImpact" control={control} render={({ field }) => (
                  <Select {...field} label="Customer Impact *" error={errors.customerImpact?.message} options={[
                    ...
                    { value: "partial-degradation", label: "Partial degradation" },
                    { value: "full-service-outage", label: "Full service outage" },
                  ]} />
                )} />
                <Controller name="businessImpact" control={control} render={({ field }) => (
                  <Input {...field} label="Business Impact (£/min)" placeholder="Optional estimate" />
                )} />
              </div>
            </div>
          </FormSection>
          ...
          <FormSection title="Assignment & Notifications">
            <div className="space-y-5">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <Controller name="assignTo" control={control} render={({ field }) => (
                  <Select {...field} label="Assign To *" error={errors.assignTo?.message} options={assigneeOptions} />
                )} />
                <Controller name="notifyChannels" control={control} render={({ field }) => (
                  <Select {...field} label="Notify Channels *" error={errors.notifyChannels?.message} options={[
                    { value: "sre-oncall-pagerduty", label: "#sre-oncall + PagerDuty" },
                    { value: "incident-room-only",   label: "Incident room only"       },
                    { value: "no-notification",       label: "No notification"          },
                  ]} />
                )} />
              </div>
              <Controller name="warRoom" control={control} render={({ field }) => (
                <WarRoomToggle value={field.value} onChange={field.onChange} />
              )} />
            </div>
          </FormSection>
          ...
          <EvidenceSection
            files={attachedFiles}
            onAdd={(f) => setAttachedFiles((p) => [...p, ...f.map((file) => ({ id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, file }))])}
            onRemove={(id) => setAttachedFiles((p) => p.filter((f) => f.id !== id))}
          />
          ...
          {createMutation.isError && (
            <p className="text-[12px] text-red-500">Unable to create the incident right now. Please try again.</p>
          )}
