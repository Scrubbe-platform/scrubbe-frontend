...
const statusStyles: Record<IncidentSidebarStatus, string> = {
  ...
  Detected: "bg-emerald-500/10 text-emerald-400 border-emerald-500/30",
  Approved: "bg-teal-500/10 text-teal-400 border-teal-500/30",
  Enriched: "bg-purple-500/10 text-purple-400 border-purple-500/30",
  Analyzed: "bg-indigo-500/10 text-indigo-400 border-indigo-500/30",
  Executing: "bg-orange-500/10 text-orange-400 border-orange-500/30",
  Resolved: "bg-slate-500/10 text-slate-300 border-slate-500/30",
  "Post-Mortem": "bg-pink-500/10 text-pink-400 border-pink-500/30",
};
    ...
    return incidents.filter((incident) => {
      ...
      const matchesStatus =
        statusFilter === "all"
          ...
          ? incident.status === "RESOLVED" || incident.sidebarStatus === "Resolved"
          : incident.status !== "RESOLVED" && incident.sidebarStatus !== "Resolved";
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div>
            ...
            <p className="text-xs text-slate-400">{row.original.elapsedLabel}</p>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-2">
            ...
            <span className="text-xs text-slate-400">
              {row.original.assignedToName || row.original.assignedToEmail || "Unassigned"}
            </span>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-sm text-slate-300">
            <p>{row.original.sourceType || "Manual"}</p>
            <p className="text-xs text-slate-500">{row.original.status}</p>
          </div>
        ),
      },
      {
    ...
    <div className="p-4 md:p-6 text-white">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold text-white">Incident Tickets</h1>
          <p className="mt-2 max-w-2xl text-sm text-slate-400">
            This route now uses the shared incident contract, so ticket list rows,
            workspace selection, and direct ticket navigation all resolve the same
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-green-500/40 px-4 py-2.5 text-sm font-semibold text-green-400 hover:bg-green-500/10"
        >
          <Plus size={16} />
          Raise Incident
        </button>
        ...
        <div className="mt-6">
          {isLoading ? (
            ...
            <Table
              ...
              columns={columns}
              onRowClick={(incident) => router.push(`/incident/tickets/${incident.id}`)}
            />
          ) : (
            <EmptyState
              title={incidents.length > 0 ? "No incidents matched this view" : "No incidents yet"}
              description={
                incidents.length > 0
              ...
              action={
                <button
                  type="button"
                  onClick={() => setOpenCreate(true)}
                  className="inline-flex items-center gap-2 rounded-lg border border-green-500/40 px-4 py-2 text-sm font-semibold text-green-400 hover:bg-green-500/10"
                >
                  <Plus size={16} />
                  Raise Incident
                </button>
      ...
      {openCreate ? <RaiseIncidentModal onClose={() => setOpenCreate(false)} /> : null}
    </div>
  );
};
