...
import { Plus, Send, Terminal, BarChart3, ListTree, ShieldAlert, History, Layout } from 'lucide-react';
...
export default function EzraConsole() {
  ...
  const { data: analysesData, isLoading: loadingAnalyses, refetch: refetchAnalyses } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.success) return (res.data?.data?.analyses ?? res.data?.data ?? []) as Analysis[];
      return [] as Analysis[];
    },
  ...
  const handleSend = async () => {
    ...
    if (!incidentId) {
      addMessage('ezra', 'Please specify an incident ID (e.g. INC-311) or select a session from the left panel.');
      return;
    }
    ...
    try {
      ...
      if (analysis.id) {
        ...
        setMessages(prev => {
          const updated = [...prev];
          updated[updated.length - 1] = { role: 'ezra', content: narrative, timestamp: new Date() };
          return updated;
        });
      }
    } catch (err: any) {
      setMessages(prev => {
        ...
        updated[updated.length - 1] = {
          role: 'ezra',
          content: err?.message ?? 'Analysis failed. Check the incident ID and try again.',
          timestamp: new Date(),
        };
        return updated;
      });
  ...
  const handleSelectAnalysis = async (analysis: Analysis) => {
    ...
    if (existing) {
      ...
      addMessage('ezra', `Loaded analysis for ${analysis.incidentId}. Switch audience view or ask a question to generate a report.`);
    }
  };
        ...
        <aside className="col-span-3 border border-white/10 rounded-2xl bg-[#030a1c] p-5 flex flex-col gap-6">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto">
            ...
            {!loadingAnalyses && analyses.length === 0 && (
              <p className="text-xs text-slate-500 text-center pt-4">No sessions yet. Ask Ezra about an incident to start.</p>
            )}
            {analyses.map(a => (
              <SessionCard
                key={a.id}
          ...
          <div className="border-t border-white/5 pt-4 space-y-4">
            <div className="space-y-2">
              ...
              <p
                ...
                onClick={() => handleQuickPrompt('Summarise the leadership impact of this incident')}
              >
                <FiMessageSquare /> Leadership view
              </p>
              <p
                className="text-[11px] text-slate-200 flex gap-1 items-center cursor-pointer hover:text-IMSCyan"
                onClick={() => handleQuickPrompt('Give me an analyst breakdown of this incident')}
              >
                <IoCodeSlashSharp /> Analyst breakdown
              </p>
              <p
                className="text-[11px] text-slate-200 flex gap-1 items-center cursor-pointer hover:text-IMSCyan"
                onClick={() => handleQuickPrompt('What is the fraud risk from this incident?')}
              >
                <FiShield /> Fraud risk
              </p>
            </div>
        ...
        <section className="col-span-5 flex flex-col border border-cyan-500/20 rounded-2xl bg-[#030a1c] relative overflow-hidden">
          <div className="flex justify-between items-center p-4 border-b border-white/5">
            <div className="flex items-center gap-2">
              <span className="text-xs">
                <span className='text-green'>ezra</span> @ <span className='text-IMSCyan'>scrubbe</span>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex items-center gap-2 rounded-md border text-sm px-2 py-0.5 transition-colors ${audience === 'LEADERSHIP' ? 'border-green text-green' : 'border-slate-600 text-slate-400 hover:border-green hover:text-green'}`}
              >
              ...
              <button
                ...
                disabled={generatingReport}
                className={`text-sm px-2 py-0.5 rounded border flex items-center gap-2 transition-colors ${audience === 'ENGINEER' ? 'border-IMSCyan text-IMSCyan' : 'border-slate-700 text-slate-400 hover:border-IMSCyan hover:text-IMSCyan'}`}
              >
                <AiOutlineLineChart size={18} /> Analyst View
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center">
                <div className="w-16 h-16 flex items-center justify-center mb-4">
                  <img src="/IMS/icons/star.svg" alt="ezrastar.svg" className="size-16" />
                </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-xl px-4 py-3 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-cyan-500/20 text-white' : 'bg-white/5 text-slate-300'}`}>
                  {msg.content}
                  <div className="text-[9px] text-slate-500 mt-1">{moment(msg.timestamp).fromNow()}</div>
                </div>
              </div>
            ))}
          ...
          <div className="p-4 bg-gradient-to-t from-black/40 to-transparent">
            <div className="relative group">
              ...
              />
              <div>
                <img src="/IMS/icons/star.svg" alt="ezrastar.svg" className="absolute left-4 top-1/2 -translate-y-1/2 text-cyan-500 size-5" />
              </div>
              <button
                onClick={handleSend}
        ...
        <aside className="col-span-4 space-y-3 overflow-y-auto pr-2">
          <div className="border border-white/10 rounded-2xl bg-[#030a1c] p-5 space-y-6">
            <div className="flex justify-between items-center">
              ...
              {selectedAnalysis ? (
                ...
                <span className="text-sm text-slate-500">No incident selected</span>
              )}
            </div>

            {selectedAnalysis ? (
              <div className="space-y-3">
                <ContextRow label="Incident" value={selectedAnalysis.incidentId} />
                <ContextRow
                  label="Service"
                  value={selectedAnalysis.situation?.affectedServices?.[0]?.name ?? '—'}
                />
                <ContextRow
                  label="Region"
              ...
              <p className="text-xs text-slate-500">Select a session or analyse an incident to see context here.</p>
            )}
            ...
            {selectedAnalysis && (
              <div className="grid grid-cols-2 gap-4">
                <MetricBox
                  label="Risk score"
                  value={selectedAnalysis.impact?.riskScore != null ? `${selectedAnalysis.impact.riskScore}/100` : '—'}
                  subValue={selectedAnalysis.impact?.severity ?? ''}
                  subValueColor={selectedAnalysis.impact?.riskScore >= 70 ? 'text-rose-500' : 'text-yellow-500'}
                />
                <MetricBox
                  label="Blast radius"
                  value={selectedAnalysis.impact?.blastRadius?.scope ?? '—'}
                  subValue={`${selectedAnalysis.impact?.affectedUsers ?? 0} users affected`}
                />
                <MetricBox
                  label="MTTR target"
                  value={selectedAnalysis.remediation?.options?.[0]?.estimatedMTTR ? `${selectedAnalysis.remediation.options[0].estimatedMTTR} min` : '—'}
                  subValue={selectedAnalysis.remediation?.options?.[0]?.title ?? ''}
                />
                <MetricBox
                  label="Confidence"
                  value={selectedAnalysis.rootCause?.confidence != null ? `${Math.round(selectedAnalysis.rootCause.confidence * 100)}%` : '—'}
          ...
          <div className="border border-white/10 rounded-2xl bg-[#030a1c] p-5">
            <div className='flex items-center justify-between'>
              ...
              <div className={`flex items-center gap-2 rounded-md border text-xs w-fit p-1 ${audience === 'LEADERSHIP' ? 'border-green text-green' : 'border-IMSCyan text-IMSCyan'}`}>
                <CiGift size={18} /> {audience === 'LEADERSHIP' ? 'Leadership' : 'Engineer'}
              </div>
            </div>
          ...
          <div className="border border-white/10 rounded-2xl bg-[#030a1c] p-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <PromptButton icon={<ListTree size={14} />} label="CI/CD" onClick={() => handleQuickPrompt('Analyse the CI/CD pipeline signals for this incident')} />
              <PromptButton icon={<BarChart3 size={14} />} label="Metrics" onClick={() => handleQuickPrompt('What metrics triggered this incident?')} />
              <PromptButton icon={<Terminal size={14} />} label="Logs" onClick={() => handleQuickPrompt('Summarise the error logs from this incident')} />
              <PromptButton icon={<ShieldAlert size={14} />} label="Fraud signals" onClick={() => handleQuickPrompt('What fraud signals are associated with this incident?')} />
              <PromptButton icon={<History size={14} />} label="Incident History" onClick={() => handleQuickPrompt('Show similar past incidents and patterns')} />
            </div>
          </div>

          <div className="border border-white/10 rounded-2xl bg-[#030a1c] p-5">
            <h3 className="text-sm font-bold text-white mb-2">Ezra response behaviour</h3>
...
const SessionCard = ({ title, subtitle, active = false, onClick }: { title: string; subtitle: string; active?: boolean; onClick?: () => void }) => (
  <div
    onClick={onClick}
    className={`p-4 rounded-xl border transition-all cursor-pointer ${active ? 'border-cyan-500/50 bg-cyan-500/5' : 'border-white/5 bg-transparent hover:bg-white/[0.02]'}`}
  >
...
const PromptButton = ({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick?: () => void }) => (
  <button
    onClick={onClick}
