  ...
  return (
    <div className="grid grid-cols-1 gap-x-6 gap-y-1 sm:grid-cols-2">
      {items.map(([label, on]) => (
        <div
          key={label}
          className="flex items-center gap-2.5 py-1.5 text-[13.5px] text-black dark:text-zinc-200"
        >
          <span
            className={cn(
...
export default function AssignmentGroups() {
  ...
  const managerOptions = useMemo(
    () => Array.from(new Set(groups.map((g) => g.manager))).sort(),
    [groups],
  );
  const deptOptions = useMemo(
    () => Array.from(new Set(groups.map((g) => g.department))).sort(),
    [groups],
  );
  const serviceOptions = useMemo(
    () => Array.from(new Set(groups.map((g) => g.primaryService))).sort(),
    [groups],
  );
  const envOptions = useMemo(
    () => Array.from(new Set(groups.map((g) => g.env))).sort(),
    [groups],
  );
    ...
    return groups.filter((g) => {
      if (
        ...
        !(
          ...
          g.primaryService.toLowerCase().includes(q)
        )
      )
        return false;
  ...
  function handleMerge(sourceId: string) {
    ...
    updateGroup(group.id, (g) => ({
      ...
      members: [
        ...g.members,
        ...src.members.filter((m) => !g.members.some((x) => x.name === m.name)),
      ],
    }));
  ...
  function handleCreate(data: CreateGroupData) {
    ...
    const newGroup: Group = {
      ...
      permissions: [
        ["Emergency Changes", false],
        ["Deployment Changes", false],
        ["Rollback", false],
        ["Production Restart", false],
      ],
    };
  ...
  return (
    <div className="flex h-screen flex-col   font-ibm dark:bg-zinc-950">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div>
            <h1 className="flex items-center gap-2.5 text-[22px] font-semibold leading-tight text-black dark:text-zinc-100">
              {!isManager && (
                <span className="rounded-md border border-zinc-200 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-black/50 dark:border-zinc-700 dark:text-zinc-500">
                  Member view
                </span>
              )}
            </h1>
            <p className="mt-1 text-[14px] text-black/60 dark:text-zinc-400">
              Operational ownership, responsibilities &amp; team structure.
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <div className="w-[160px] shrink-0">
            <Select
              ...
              options={[
                { value: "", label: "Manager" },
                ...managerOptions.map((o) => ({ value: o, label: o })),
              ]}
              placeholder="Manager"
            />
          </div>
          <div className="w-[160px] shrink-0">
            <Select
              ...
              options={[
                ...
                ...deptOptions.map((o) => ({ value: o, label: o })),
              ]}
          ...
          <div className="w-[150px] shrink-0">
            <Select
              ...
              options={[
                ...
                ...envOptions.map((o) => ({ value: o, label: o })),
              ]}
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-hidden p-4 sm:p-5 lg:grid-cols-[360px_1fr]">
        ...
        <aside
          className={cn(
            "flex min-h-0 flex-col overflow-hidden rounded-lg bg-white shadow-sm shadow-light dark:bg-zinc-900/40",
            showProfileMobile && "hidden lg:flex",
          )}
        >
          <div className="flex items-center gap-2 border-b border-zinc-100 px-4 py-3.5 dark:border-zinc-800">
            ...
            {!isManager && (
              <span className="ml-auto text-[12px] text-black/50 dark:text-zinc-500">
                Your teams
              </span>
            )}
          </div>
        ...
        <main
          className={cn(
            "min-h-0 overflow-y-auto rounded-lg bg-zinc-50 dark:bg-zinc-900/20",
            !showProfileMobile && "hidden lg:block",
          )}
        >
          {!group ? (
            ...
            <div className="mx-auto max-w-5xl px-5 py-5 pb-14 sm:px-7">
              <button
                ...
                className="mb-3 flex items-center gap-1.5 text-[12px] text-black hover:text-emerald-600 dark:text-zinc-400 lg:hidden"
              >
              ...
              <div className="rounded-lg bg-white p-7 shadow-sm shadow-light dark:bg-zinc-900/60">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mb-2 text-lg font-bold leading-tight text-black dark:text-zinc-100">
                      {group.name}
                    </h2>
                  ...
                  {isManager && (
                    <button
                      onClick={() => setModal({ kind: "editGeneral" })}
                      className="rounded-md border border-zinc-300 px-3 py-1.5 text-[12px] text-black hover:border-emerald-400 hover:text-emerald-600 dark:border-zinc-700 dark:text-zinc-200"
                    >
                      Edit
                    </button>
                  )}
              ...
              <div className="mt-4 rounded-lg bg-white p-7 shadow-sm shadow-light dark:bg-zinc-900/60">
                    ...
                    <div className="mt-3.5 flex items-start gap-2.5 rounded-md bg-zinc-50 px-3.5 py-3 text-[12.5px] text-black dark:bg-zinc-900/60 dark:text-zinc-300">
                      ...
                      <p>
                        <b className="font-semibold text-black dark:text-zinc-100">
                          Ezra is a governed service identity.
                        </b>{" "}
                        It can be assigned incidents in this team and acts under
                        delegated, approval-gated authority — every action is
                        RBAC-scoped and audited. It never counts toward human
                        on-call or Incident Commander coverage.
                      </p>
                    </div>
                  )}
                  {isManager && (
                ...
                <SectionCard
                  eyebrow="Escalation Policy"
                  right={
                    isManager && (
                      <button
                        onClick={() => setModal({ kind: "escalation" })}
                        className="text-[12px] font-medium text-emerald-600 hover:underline dark:text-emerald-400"
                      >
                        Edit path →
                      </button>
                    )
                ...
                <SectionCard
                  eyebrow="Operational Expertise"
                  note="Signals for responder assignment"
                >
                  <div className="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-3">
                    {group.expertise.map(([name, n]) => (
                      <div key={name} className="flex items-center gap-3">
                        <span className="text-[13.5px] text-black dark:text-zinc-200">
                          {name}
                        </span>
                        <Stars n={n} />
                      </div>
                  ...
                  <SectionCard eyebrow="Admin Actions">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="rounded-md border border-rose-200 px-3 py-1.5 text-[12px] font-medium text-rose-600 hover:bg-rose-50 dark:border-rose-500/30 dark:text-rose-400 dark:hover:bg-rose-500/10"
                      >
                        Delete group
                      </button>
                    </div>
      ...
      <Modal isOpen={modal.kind !== "none"} onClose={closeModal}>
        ...
        {group && modal.kind === "merge" && (
          <MergeForm
            ...
            others={groups.filter(
              (g) => g.id !== group.id && g.status === "Active",
            )}
            onSave={handleMerge}
            onCancel={closeModal}
          />
        )}
        {group && modal.kind === "delete" && (
          <DeleteConfirm
            name={group.name}
            onCancel={closeModal}
        ...
        {modal.kind === "create" && (
          <CreateGroupForm
            defaultManager={currentUserName}
            onCreate={handleCreate}
            onCancel={closeModal}
          />
        )}
        {group && modal.kind === "escalation" && (
          <EscalationEditor
            levels={group.escalation}
            onSave={handleEscalationSave}
            onCancel={closeModal}
          />
        )}
        {group && modal.kind === "oncall" && (
          <OnCallManagerForm
            group={group}
            onSave={handleOncallSave}
            onCancel={closeModal}
          />
        )}
        {group && modal.kind === "editGeneral" && (
          <EditGeneralForm
            group={group}
            onSave={handleEditGeneral}
            onCancel={closeModal}
          />
        )}
        ...
        {group && modal.kind === "ezra" && (
          <EzraProfileModal
            group={group}
            isManager={isManager}
            onToggle={handleEzraToggle}
          />
        )}
        {group && modal.kind === "item" && (
          <ItemReferenceModal
            kind={modal.itemKind}
            name={modal.name}
            group={group}
