    ...
    <div className="grid grid-cols-1 gap-x-6 sm:grid-cols-2">
      {rows.map(([k, v, wide], i) => (
        <div
          key={i}
          className={cn(
            "border-b border-zinc-100 py-3.5 last:border-b-0 dark:border-zinc-800",
            wide && "sm:col-span-2",
          )}
        >
          <div className="mb-0.5 text-[13.5px] text-black/55 dark:text-zinc-500">
            {k}
          </div>
          <div className="text-[15px] font-semibold text-black dark:text-zinc-200">
            {v}
          </div>
        </div>
      ))}
  ...
  return (
    <span className="tracking-[2px] text-emerald-600 dark:text-emerald-400">
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
          className={i > n ? "text-zinc-200 dark:text-zinc-700" : ""}
        >
          ★
        </span>
      ))}
...
const riskText: Record<Tone, string> = {
...
export function ReadinessBlock({
  ...
  const item = (label: string, val: string) => (
    <div key={label}>
      <div className="text-[13px] text-black/50 dark:text-zinc-500">
        {label}
      </div>
      <div className="mt-1 text-[15px] text-black dark:text-zinc-100">
        {val}
      </div>
    </div>
  );
  ...
  if (!rd.availOk)
    gaps.push(`only ${rd.available}/${rd.total} members available`);
  if (!rd.skills) gaps.push("expertise gaps");
  const alts = allGroups
    .filter(
      (o) =>
        o.id !== group.id &&
        o.status === "Active" &&
        readiness(o).risk === "low" &&
        readiness(o).ic,
    )
    .map((o) => o.name)
    .slice(0, 2);
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900/40">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <h4 className="text-lg font-bold text-black dark:text-zinc-100">
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-3">
        ...
        {group.ezra &&
          item(
            "Ezra · service identity",
            ezOn
              ? `Assignable${group.ezra.assigned ? ` · ${group.ezra.assigned} assigned` : ""}`
              : "Paused",
          )}
      </div>
      <div className="mt-6 flex items-start gap-3 rounded-lg bg-zinc-50 p-4 dark:bg-zinc-900/60">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-zinc-900 text-[13px] font-semibold text-white dark:bg-zinc-700">
          E
        </span>
        {rd.risk !== "low" ? (
          <p className="text-[13.5px] font-medium text-black/80 dark:text-zinc-200">
            <b className="font-bold">Ezra:</b> {group.name} has{" "}
            {gaps.join(", ")}.{" "}
            {alts.length ? (
              <>
                For a major incident right now, route to{" "}
                <b className="font-bold">{alts.join("</b> or <b>")}</b>.
              </>
            ) : (
          ...
          <p className="text-[13.5px] font-medium text-black/80 dark:text-zinc-200">
            <b className="font-bold">Ezra:</b> {group.name} is ready to take a
            major incident — coverage, on-call and expertise all check out.
          </p>
        )}
      </div>
    </div>
      ...
      <table className="w-full min-w-[560px] border-collapse text-[13px]">
        <thead>
          <tr className="border-b border-zinc-200 dark:border-zinc-700">
            {["Name", "Role", "Availability", "On-call", "Incidents", ""].map(
              (h) => (
                <th
                  key={h}
                  className="px-0 pb-2 pr-3 text-left text-[10px] font-semibold uppercase tracking-wide text-black dark:text-zinc-500"
                >
                  {h}
                </th>
              ),
            ...
            >
              <td className="py-2.5 pr-3">
                <button
                  ...
                  className="text-left font-medium text-black hover:text-emerald-600 hover:underline dark:text-zinc-100 dark:hover:text-emerald-400"
                >
                  {m.name}
                </button>
              </td>
              <td className="py-2.5 pr-3 text-black dark:text-zinc-300">
                {m.role}
              </td>
              <td className="py-2.5 pr-3">
              ...
              <td className="py-2.5 text-right">
                {isManager && (
                  <button
                    onClick={() => onRemove(i)}
                    title="Remove member"
                    className="rounded-md px-1.5 py-0.5 text-black hover:bg-rose-50 hover:text-rose-600 dark:text-zinc-500 dark:hover:bg-rose-500/10 dark:hover:text-rose-400"
                  >
                    <X size={14} />
                  </button>
                )}
            ...
            <tr className="border-b border-zinc-100 bg-emerald-50/60 dark:border-zinc-800 dark:bg-emerald-500/5">
              <td className="border-l-2 border-l-emerald-500 py-2.5 pl-2 pr-3">
                <span className="inline-flex items-center gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-emerald-600 text-[11px] font-semibold text-white">
                    E
                  </span>
                  <button
                    onClick={onEzraClick}
                    className="font-medium text-black hover:text-emerald-600 hover:underline dark:text-zinc-100 dark:hover:text-emerald-400"
                  >
                    Ezra
                  </button>
                  <span className="whitespace-nowrap rounded-md border border-emerald-200 px-1.5 py-0.5 font-mono text-[9px] uppercase text-emerald-700 dark:border-emerald-500/30 dark:text-emerald-400">
                    AI · Service identity
                  </span>
                </span>
              </td>
              <td className="py-2.5 pr-3 text-black dark:text-zinc-300">
                Governed response agent
              </td>
              <td className="py-2.5 pr-3">
                <span className="inline-flex items-center gap-1.5 text-[12.5px] text-black dark:text-zinc-300">
      ...
      {levels.map((lv, i) => (
        <React.Fragment key={i}>
          <div className="rounded-lg border border-zinc-200 bg-white px-4 py-3.5 dark:border-zinc-700 dark:bg-zinc-900/60">
            ...
            <div className="mt-0.5 text-[15px] font-semibold text-black dark:text-zinc-200">
              {lv}
            </div>
          </div>
          {i < levels.length - 1 && (
            <div className="flex h-6 items-center justify-center text-black/30 dark:text-zinc-500">
              ↓
            </div>
          )}
        </React.Fragment>
...
export function OnCallCard({
  ...
  oncall: { name: string | null; ends: string | null };
    ...
    return (
      <div className="flex max-w-md items-center justify-between gap-4 rounded-lg border border-rose-200 bg-rose-50 p-4 dark:border-rose-500/30 dark:bg-rose-500/5">
        <p className="text-[13px] font-medium text-rose-600 dark:text-rose-400">
          No one currently on call — coverage gap.
        </p>
        {action}
      </div>
    );
  ...
  return (
    <div className="flex max-w-md items-center justify-between gap-4 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900/60">
      <div>
        <div className="text-[18px] font-semibold text-black dark:text-zinc-100">
          {oncall.name}
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <span className="truncate text-[15px] font-bold text-black dark:text-zinc-100">
          {group.name}
        </span>
        <span
          className={cn(
