...
function getWorkspaceSubdomain() {
  ...
  if (!hostname.endsWith(".scrubbe.com")) return null;

  const subdomain = hostname.replace(/\.scrubbe\.com$/, "");
...
export default function ContactAdminPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      setIsLoading(true);
      const workspace = getWorkspaceSubdomain();
      if (workspace) {
        const res = await apiClient.post(
          `${endpoint.auth.workspace_access_check}/${encodeURIComponent(workspace)}/access-check`,
          {
        ...
        const result = responseData<{ redirectUrl?: string }>(res);
