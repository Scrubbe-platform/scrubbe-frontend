    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col gap-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-black dark:text-zinc-500 truncate">{service}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  return (
    <div className="flex h-full w-72 flex-col gap-5 overflow-y-auto border-r border-zinc-100 dark:border-zinc-800 bg-white dark:bg-transparent p-5">
      ...
      <div className="flex items-center justify-between">
        <p className="text-[13px] font-semibold text-black dark:text-zinc-100">Playbooks</p>
          ...
          {legends.map(({ id, label, status }) => (
            <div key={id} className="flex items-center justify-between group cursor-default">
              ...
              {status === "completed" ? (
                ...
                <Circle size={14} className="text-zinc-200 dark:text-black shrink-0" />
              )}
            </div>
          ))}
        </div>
