...
const ModeCard = ({ title, description, stage, status }: ExecutionModeCard) => (
  <div
    className={cn(
      ...
      status === "default" &&
        "border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 opacity-60",
      status === "locked" &&
        "border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 opacity-40",
    )}
  >
    <div
      className={cn(
    ...
    <h4 className="text-[13px] font-semibold text-black dark:text-zinc-100 mb-1.5">
      {title}
    </h4>
    <p className="text-[11px] text-black dark:text-zinc-500 leading-relaxed mb-3 max-w-[180px]">
      {description}
    </p>
    <span className="text-[10px] font-semibold uppercase tracking-wider text-black dark:text-zinc-400">
      {stage}
    </span>
  </div>
);
...
const ExecutionGate: React.FC<{ incidentId?: string }> = ({ incidentId }) => {
  ...
  const { data: pendingDecision } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.success) {
        ...
        return (
          decisions.find(
            (d: any) => d.status === "PENDING" || d.requiresApproval,
          ) ?? null
        );
      }
      return null;
  ...
  const { mutateAsync: proposeRollback, isPending: proposing } = useMutation({
    mutationFn: async () => {
      ...
      if (!res.success)
        throw new Error(res.data ?? "Failed to propose decision");
      return res.data;
    },
    onSuccess: () => {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-800 rounded-xl">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[12px] font-semibold text-black dark:text-zinc-200">
            Execution Mode
          </span>
        ...
        <p className="text-[11px] font-mono text-black dark:text-zinc-500">
          ...
          <span className="font-semibold text-zinc-600 dark:text-zinc-300 ml-1">
            {ealLevel} · PROPOSE
          </span>
        </p>
      </div>
      ...
      <div className="p-4 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-500/20 rounded-xl flex gap-3">
        <Lock
          size={14}
          className="text-red-400 dark:text-red-500 shrink-0 mt-0.5"
        />
        <p className="text-[12px] text-red-600 dark:text-red-400 leading-relaxed">
          ...
          <span className="font-mono bg-red-100 dark:bg-red-500/10 px-1 rounded text-[11px]">
