...
const ModeCard = ({ title, description, stage, status }: ExecutionModeCard) => {
  ...
  return (
    <div
      className={`flex-1 flex flex-col items-center text-center p-8 rounded-2xl border transition-all ${styles[status]}`}
    >
      <div
        className={`p-3 rounded-lg mb-4 border border-white/10 ${
          status === "active" ? "bg-emerald-500/10" : "bg-white/5"
        }`}
      >
        <Users
          size={24}
          className={
            status === "active" ? "text-emerald-500" : "text-slate-500"
          }
        />
      </div>
      ...
      <span
        className={`text-[11px] font-bold uppercase tracking-wider ${textStyles[status]}`}
      >
        {stage}
      </span>
    </div>
...
const ExecutionGate: React.FC = () => {
  const modes: ExecutionModeCard[] = [
    {
      title: "Manual",
      description: "Engineer runs action directly. No assistance.",
      stage: "Stage 1",
      status: "default",
    },
    {
      title: "Assisted",
      description: "Scrubbe prepares. Engineer confirms.",
      stage: "Stage 2 · ACTIVE",
      status: "active",
    },
    {
      title: "Automated",
      description: "Scrubbe executes when risk is low.",
  ...
  return (
    <div className="w-full max-w-6xl bg-dark border border-white/5 rounded-3xl p-3 flex flex-col gap-8">
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1 font-mono">
              CP enforced · effectiveAutomationLevel = min(playbook, policy,
              risk) · block on uncertainty
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <div className="px-4 py-2 border border-amber-500/40 bg-amber-500/5 rounded text-amber-500 text-xs font-bold tracking-widest">
            AWAITING APPROVAL
          </div>
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white/[0.02] border border-white/5 rounded-2xl">
        ...
        <div className="flex items-center gap-4">
          <p className="text-xs font-mono text-slate-500">
            ...
            <span className="text-purple-400 font-bold ml-2">2 · PROPOSE</span>
          </p>
      ...
      {/* Prepare Rollback CTA */}
      <button className="w-full flex items-center justify-center gap-3 py-4 bg-emerald-500/5 border border-emerald-500/30 rounded-xl text-emerald-500 hover:bg-emerald-500/10 transition-all group">
        <CheckCircle2
          size={20}
          className="group-hover:scale-110 transition-transform"
        />
        <span className="text-sm font-semibold tracking-wide">
          Prepare Rollback for engineer approval
        </span>
      </button>

      {/* CP Enforcement Warning Box */}
      <div className="p-5 bg-red-950/20 border border-red-500/20 rounded-xl flex gap-4">
        ...
        <p className="text-sm text-red-400 leading-relaxed font-medium">
          CP Enforcement: if policy verification is unavailable during network
          partition, execution is blocked. Outcome will be recorded as:{" "}
          <span className="font-mono text-[13px] bg-red-500/10 px-1 rounded">
