...
const typeStyle: Record<ImpactType, string> = {
  Direct:
    "border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800",
  Cascade:
    "border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800",
  InDirect:
    "border-zinc-200 dark:border-zinc-700 text-black dark:text-zinc-500 bg-zinc-50 dark:bg-zinc-800/60",
};
...
const ImpactCard = ({ name, subtext, type }: ServiceImpact) => (
  <div className="flex items-center justify-between p-4 bg-white dark:bg-zinc-900/40 border border-zinc-100 dark:border-zinc-800 rounded-xl hover:border-zinc-200 dark:hover:border-zinc-700 transition-colors">
    ...
    <span
      className={`text-[10px] font-semibold px-2 py-0.5 rounded border uppercase tracking-wider ${typeStyle[type]}`}
    >
      {type}
    </span>
  </div>
...
const BlastRadiusModule: React.FC<{ incidentId?: string }> = ({
  ...
  const { data: report } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await get(
        `${endpoint.blast_radius.getForTrigger}/${incidentId}`,
      );
    ...
    >
      ...
      <div className="flex gap-3 rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-4">
        ...
        <p className="text-[12px] text-black dark:text-zinc-400 leading-relaxed">
          ...
          — blast radius must be calculated and consistent before any guardrail
          or execution decision. Uncertainty blocks execution.
        </p>
      </div>
