...
const AutoCreateIncident = () => {
  ...
  const { data: rules = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.success)
        return (res.data?.data?.guardrails ?? res.data?.data ?? []) as any[];
      return [] as any[];
    },
  ...
  const { mutateAsync: createRule, isPending: saving } = useMutation({
    mutationFn: async (data: TformScheme) => {
      ...
      if (!res.success)
        throw new Error(res.data?.message ?? "Failed to save rule");
      return res.data;
    },
    onSuccess: () => {
      ...
      >
        <div className="space-y-3">
          ...
          <div className="border border-gray-400 p-3 rounded-lg space-y-3">
            <div className="flex justify-between">
              <div>
                ...
                <p className="text-xs">
                  Avoid duplicate incidents for the same service & deploy.
                </p>
              </div>
            </div>
            ...
            <Select
              options={[
                {
                  value: "Service + env ( default )",
                  label: "Service + env ( default )",
                },
                {
                  value: "Service + env + pipeline",
                  label: "Service + env + pipeline",
                },
          ...
          <div className="border border-gray-400 p-3 rounded-lg space-y-3">
            ...
            <p className="text-xs">
              Sets the resolution countdown per severity.
            </p>
          ...
          <div className="border border-gray-400 p-3 rounded-lg space-y-3">
            ...
            <p className="text-xs">
              Policy for what remediation paths are allowed by default.
            </p>
          ...
          <div>
            <div className="border border-gray-300 rounded-md p-2">
              ...
              <p className="pt-2 text-sm">
                Evaluates triggers (pipelines/alerts/risk) to auto-create
                incidents.
              </p>
            </div>
            ...
            <div className="mt-4">
              <div className="grid grid-cols-2 gap-x-3">
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      {...field}
                      placeholder="Pipeline fail in prod - P1"
                      label="Name"
                      labelClassName="text-white"
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      options={[
                        ...
                        {
                          value: "Monitoring alert",
                          label: "Monitoring alert",
                        },
                        {
                          value: "Fraud / risk engine",
              ...
              <div className="mt-3">
                <Controller
                  ...
                  render={({ field }) => (
                    <TextArea
                      {...field}
                      placeholder="Explain intent so future readers don't guess"
                      label="Note (optional)"
                      labelClassName="text-white"
                    />
                  )}
              ...
              <div className="flex items-center gap-2 text-sm mt-2">
                <BiInfoCircle className="size-4" />
                <p>
                  Rules are evaluated in order. Put stricter rules above looser
                  ones.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-3 mt-3">
                <CButton
                  ...
                  className="border bg-transparent hover:bg-transparent border-IMSCyan text-IMSCyan w-fit"
                >
                  Cancel
                </CButton>
                <CButton
