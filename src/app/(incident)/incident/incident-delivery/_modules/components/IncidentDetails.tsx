      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <section>
          <div className="flex justify-between items-start mb-8">
            <div className="space-y-2">
              ...
              <p className=" text-slate-400 font-medium tracking-wide">
                Raised automatically (delivery incident)
              </p>
              <p className="text-base text-slate-500 italic">
                Dedup keeps one incident per correlated failure.
              </p>
            </div>
          ...
          <div className="bg-black border border-slate-800 rounded-2xl p-6">
            <h3 className="text-sm font-black text-white tracking-widest mb-3">
              Incident title
            </h3>
            <p className="text-sm font-semibold text-slate-200">
              Merge conflict detected — Add new policy evaluator
            </p>
          </div>
        </section>
      </div>
  ...
  <div className="bg-black border border-slate-800 rounded-2xl p-6 flex flex-col gap-2 group hover:border-green-500/30 transition-colors">
    ...
    {list &&
      list.map((item: string, i: number) => (
        <p key={i} className="text-sm text-slate-400 font-medium">
          {item}
        </p>
      ))}
  </div>
