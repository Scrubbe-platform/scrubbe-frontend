...
import {
  ...
  PlusSquare, ArrowUpRight, Minus, Link,
} from "lucide-react";
...
const Remediation: React.FC<{ incidentId?: string }> = ({ incidentId }) => {
  ...
  const suggestion = analysis?.remediation?.suggested ?? analysis?.remediation?.action ?? null;
  const prSuggestion = analysis?.remediation?.prChange ?? null;
  const ciSuggestion = analysis?.remediation?.ciAction ?? null;
  const confidence = analysis?.remediation?.confidence ?? null;
      ...
      <div className="w-full">
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="space-y-1">
            ...
            <p className=" text-slate-300 font-medium mt-4">Suggest → verify → summarize</p>
        ...
        <div className="bg-black border border-slate-500 rounded-2xl p-6 mb-6">
          ...
          <div className="flex gap-2 mb-6">
            <StatusPill label={`Confidence: ${confidence !== null ? confidence : "—"}`} />
            <StatusPill label="PR: —" />
            <StatusPill label="CI: —" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-black border border-slate-500 rounded-2xl p-6">
            ...
            <div className="h-40 bg-black/60 rounded-xl border border-slate-800 p-4">
              <span className="text-slate-600 font-mono">{suggestion ?? "—"}</span>
            </div>
          </div>
          ...
          <div className="bg-black border border-slate-500 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-400">{analysis?.reports?.find((r: any) => r.audience === "LEADERSHIP")?.narrative ?? "—"}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase">Analyst</p>
                <p className="text-xs text-slate-400">{analysis?.reports?.find((r: any) => r.audience === "ENGINEER")?.narrative ?? "—"}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-black border border-slate-500 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="px-4 py-1.5 border border-green-500 text-green-500 rounded-lg text-xs font-bold">Expand</button>
          </div>
...
const StatusPill: React.FC<{ label: string }> = ({ label }) => (
  <div className="px-3 py-1 bg-black border border-slate-800 rounded-full text-[10px] font-mono text-slate-400">{label}</div>
);
