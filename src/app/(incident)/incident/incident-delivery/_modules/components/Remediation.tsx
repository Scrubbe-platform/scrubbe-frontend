...
import {
  Sparkles, Code, PlayCircle, Zap, Info,
  PlusSquare, ArrowUpRight, Link,
} from "lucide-react";
...
interface Hypothesis { id: string; title: string; confidence: number; runUrl: string; diffUrl: string }

// ── Actions data ──────────────────────────────────────────────────

const actions = [
  { icon: <Sparkles size={13} />,    label: "Generate Suggestion",    variant: "amber"   },
  { icon: <Code size={13} />,        label: "Review in Code Engine",  variant: "amber"   },
  { icon: <PlayCircle size={13} />,  label: "Execute safe action",    variant: "red"     },
  { icon: <Zap size={13} />,         label: "Generate Ezra",          variant: "red"     },
  { icon: <Info size={13} />,        label: "Suggestion details",     variant: "sky"     },
  { icon: <PlusSquare size={13} />,  label: "Create PR",              variant: "violet"  },
  { icon: <Zap size={13} />,         label: "Merge suggested PR",     variant: "red"     },
  { icon: <ArrowUpRight size={13} />,label: "Open Ezra full view",    variant: "emerald" },
];
...
const Remediation: React.FC<{ incidentId?: string }> = ({ incidentId }) => {
  ...
  const rawHypotheses: any[] = analysis?.rootCause?.hypotheses ?? analysis?.situation?.hypotheses ?? analysis?.remediation?.options ?? [];
  const hypotheses: Hypothesis[] = rawHypotheses.length
    ? rawHypotheses.map((h: any, i) => ({ id: h.id ?? `HYP-${i + 1}`, title: h.title ?? h.description ?? String(h), confidence: h.confidence ?? h.score ?? 0.7, runUrl: h.runUrl ?? "#", diffUrl: h.diffUrl ?? "#" }))
    : [{ id: "HYP-1", title: "Waiting for connector hypotheses", confidence: 0.4, runUrl: "#", diffUrl: "#" }];

  const primaryOption = analysis?.remediation?.options?.[0] ?? null;
  const suggestion    = analysis?.remediation?.recommendedAction ?? analysis?.remediation?.suggested ?? analysis?.remediation?.action ?? "Waiting for a connector remediation suggestion";
  const prSuggestion  = analysis?.remediation?.prChange ?? primaryOption?.prChange ?? "No PR patch has been generated yet.";
  const ciSuggestion  = analysis?.remediation?.ciAction ?? primaryOption?.ciAction ?? "No CI verification action is available yet.";
  const confidence    = analysis?.remediation?.confidence ?? analysis?.rootCause?.confidence ?? null;
  const playbook      = analysis?.remediation?.playbook ?? primaryOption?.playbook ?? null;
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {actions.map(({ icon, label, variant }) => (
            <button key={label} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-medium transition-colors hover:opacity-80 ${actionVariant[variant]}`}>
              {icon} {label}
            </button>
          ))}
        </div>
        ...
        <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { title: "Suggested action",    main: suggestion,    note: playbook ? `Playbook: ${playbook}` : "Generate a connector-informed remediation PR before rerunning verification." },
              { title: "Suggested PR change", main: prSuggestion,  note: 'Files touched and risk should align with the "Diff provenance" review.'                                         },
          ...
          <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { label: "Flake rate",    value: primaryOption?.safetyLevel ?? "--"        },
