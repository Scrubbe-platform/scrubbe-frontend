...
export function HealthBadge({ health }: { health: Health }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11.5px] font-semibold", HEALTH_STYLE[health])}>
...
export function CardHeader({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="mb-3.5 flex flex-wrap items-baseline justify-between gap-2">
      <h3 className="text-[14.5px] font-bold text-black dark:text-zinc-100">{title}</h3>
      {hint && <span className="text-[12px] text-black/40 dark:text-zinc-500">{hint}</span>}
    </div>
  );
}
    ...
    <div>
      {rows.map((row, i) => (
        ...
        >
          ...
          <span className={cn("font-semibold text-black dark:text-zinc-100", row.tone && TONE[row.tone])}>{row.v}</span>
        </div>
      ))}
    </div>
  );
        ...
        <li key={it.label} className="flex items-start gap-2.5 text-[13px]">
          <span
            className={cn(
              ...
              it.ok ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400",
            )}
          >
            {it.ok ? "✓" : "✕"}
          </span>
          <span>
            <span className="font-medium text-black dark:text-zinc-200">{it.label}</span>
            {it.note && <span className="block text-[12px] text-black/50 dark:text-zinc-500">{it.note}</span>}
          </span>
        </li>
      ))}
    </ul>
...
export function StatePill({ label, cls }: { label: string; cls: "ok" | "warn" | "bad" }) {
  ...
  const DOT: Record<string, string> = { ok: "bg-emerald-500", warn: "bg-amber-500", bad: "bg-rose-500" };
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11.5px] font-semibold", STYLE[cls])}>
...
export function MiniStat({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "ok" | "warn" | "bad" }) {
  const TONE: Record<string, string> = {
    ok: "text-emerald-700 dark:text-emerald-400",
    warn: "text-amber-700 dark:text-amber-400",
    bad: "text-rose-700 dark:text-rose-400",
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-900">
      <div className="text-[10.5px] font-semibold uppercase tracking-wide text-black/40 dark:text-zinc-500">{label}</div>
      <div className={cn("mt-1 font-ibm text-[15px] font-bold text-black dark:text-zinc-100", tone && TONE[tone])}>{value}</div>
    </div>
  );
}
...
export function ReadinessRing({ score, band, size = 112 }: { score: number; band: string; size?: number }) {
  const r = (size - 20) / 2;
  const c = 2 * Math.PI * r;
  const color = band === "Ready" ? "#059669" : band === "Conditional" ? "#d97706" : "#e11d48";
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#E7EAF0" strokeWidth={10} />
      <circle
        cx={size / 2}
        cy={size / 2}
        r={r}
      ...
      />
      <text x={size / 2} y={size / 2 + 6} textAnchor="middle" fontFamily="monospace" fontSize={size / 4.3} fontWeight={700} fill="#0B1220">
        {score}
      </text>
      <text x={size / 2} y={size / 2 + 20} textAnchor="middle" fontFamily="inherit" fontSize={10} fill="#8B93A1">
        of 100
      </text>
    </svg>
  );
