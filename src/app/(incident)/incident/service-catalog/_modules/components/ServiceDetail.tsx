...
import { ServiceRecord, ealOf } from "./serviceCatalog.data";
import {
  NAV_SECTIONS,
  CurrentStateSection,
...
export default function ServiceDetail({
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const els = NAV_SECTIONS.map(([id]) => root.querySelector<HTMLElement>(`#s-${id}`)).filter(
  ...
  return (
    <div ref={rootRef} className="mx-auto max-w-[1600px] p-4 font-ibm sm:p-6">
      <div className="mb-2 flex items-center gap-1.5 text-[12.5px] text-black/40 dark:text-zinc-500">
        <button onClick={onBack} className="hover:text-black dark:hover:text-zinc-200">
          Overview
        </button>
        <ChevronRight size={13} />
        <span className="font-semibold text-black dark:text-zinc-200">{service.name}</span>
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-[24px] font-bold leading-tight text-black dark:text-zinc-100">{service.name}</h1>
            ...
            <p className="mt-2.5 max-w-2xl text-[13px] leading-relaxed text-black/60 dark:text-zinc-400">
              Handles {service.name.toLowerCase()} operations in {service.env.toLowerCase()}, running
              on {service.runtime} in {service.cloud} ({service.region}).
            </p>
            <div className="mt-4 flex flex-wrap gap-x-7 gap-y-3">
              <Meta k="Service ID" v={service.id} mono />
              <Meta k="Owner team" v={service.owner} />
              <Meta k="Version" v={`v${service.version}`} mono />
              <Meta k="Created" v={service.createdYearsAgo === 0 ? "Just now" : `${service.createdYearsAgo} yr${service.createdYearsAgo > 1 ? "s" : ""} ago`} />
              <Meta k="Language" v={service.lang} />
            </div>
          </div>
          <div className="flex shrink-0 gap-2">
            <button
              onClick={() => toast.info(`Running a health scan for ${service.name}…`)}
            ...
            <button
              onClick={() => toast.info("Edit service form is coming in a later update")}
      ...
      <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[220px_1fr]">
        ...
        <div className="flex min-w-0 flex-col gap-5">
          ...
          <DependenciesSection service={service} onOpenService={onOpenService} />
...
function Meta({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div>
      ...
      <div className={cn("mt-0.5 text-[13px] font-semibold text-black dark:text-zinc-100", mono && "font-ibm")}>{v}</div>
    </div>
  );
}
