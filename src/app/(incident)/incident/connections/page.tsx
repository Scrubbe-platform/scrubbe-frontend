...
const fraudModel = [
  ...
  },
  {
    name: "events-stream",
    title: "Events / stream",
    subtitle: "Kafka / Kinesis / Pub-sub",
    Icon: LuPuzzle,
  },
  {
    name: "third-party",
...
const Page = () => {
  ...
  const { data: integrations = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (res.data?.data ??
        ...
        res.data ??
        []) as Array<{ provider: string; isActive: boolean }>;
    },
    enabled: !!user?.id,
  });
  ...
  const connectedProviders = useMemo(() => {
    ...
    integrations.forEach((i) => {
      if (i.isActive) set.add(i.provider.toLowerCase());
    });
    connections.forEach((c) => {
      if (c.status === "HEALTHY" || c.status === "CONNECTING")
        set.add(c.type.toLowerCase());
    });
    return set;
  }, [integrations, connections]);

  const isConnected = (name: string) =>
    connectedProviders.has(name.toLowerCase());
    ...
    <div className="bg-white dark:bg-zinc-950 min-h-screen text-black dark:text-zinc-200 font-sans p-10">
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          {[
            {
              icon: <BiGitRepoForked size={13} />,
              label: "Step 1 of 3 · Engineering graph",
            },
            {
              icon: <GoShield size={13} />,
              label: "Read-only integration model",
            },
          ].map(({ icon, label }) => (
            <div
              key={label}
              className="px-3 py-1.5 flex items-center gap-2 border border-zinc-500 dark:border-zinc-700 text-[11px] font-medium text-zinc-600 dark:text-zinc-300 rounded-md"
            >
              {icon} {label}
            </div>
          ))}
        ...
        <div className="flex gap-10">
          <div className="space-y-6 flex-1">
              ...
              <div className="grid grid-cols-3 gap-4">
                {integrationModel.map(({ Icon, ...rest }) => (
                  <IntegrationCard
                    ...
                    connected={isConnected(rest.name)}
                    onClick={() =>
                      openModal("code_repos", rest.title, rest.name)
                    }
                  />
                ))}
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {deploymentModel.map(({ Icon, ...rest }) => (
                  <IntegrationCard
                    ...
                    subtitle={rest.subtitle}
                    connected={isConnected(rest.name)}
                    onClick={() => openModal("cicd", rest.title, rest.name)}
                  />
                ))}
              </div>
            </StepWrapper>
              ...
              <div className="grid grid-cols-3 gap-4">
                {containersModel.map(({ Icon, ...rest }) => (
                  <IntegrationCard
                    ...
                    subtitle={rest.subtitle}
                    connected={isConnected(rest.name)}
                    onClick={() => openModal("runtime", rest.title, rest.name)}
                  />
                ))}
              </div>
            </StepWrapper>
              ...
              <div className="grid grid-cols-3 gap-4">
                {databaseModel.map(({ Icon, ...rest }) => (
                  <IntegrationCard
                    ...
                    connected={isConnected(rest.name)}
                    onClick={() =>
                      openModal("datastores", rest.title, rest.name)
                    }
                  />
                ))}
              </div>
            ...
            <StepWrapper
              ...
              footer='Fraud integrations are optional, but if you are a payments or fintech business this is where Scrubbe becomes "fraud-aware", not just "infra-aware".'
            >
              <div className="grid grid-cols-3 gap-4">
                {fraudModel.map(({ Icon, ...rest }) => (
                  <IntegrationCard
                    ...
                    connected={isConnected(rest.name)}
                    onClick={() =>
                      openModal("fraud_metrics", rest.title, rest.name)
                    }
                  />
                ))}
              </div>
          ...
          <aside className="flex-[0.5] space-y-6">
            {[
              ...
              <div
                key={title}
                className="border border-zinc-500 dark:border-zinc-700/60 bg-white dark:bg-zinc-900/40 p-6 rounded-xl"
              >
                <h3 className="text-[14px] font-semibold text-black dark:text-zinc-100 mb-3">
                  {title}
                </h3>
                <ul className="list-disc pl-4 space-y-2">
                  {items.map((item) => (
                    <li
                      key={item}
                      className="text-[13px] text-black dark:text-zinc-400 leading-relaxed"
                    >
                      {item}
                    </li>
                  ))}
  ...
  <div className="border border-zinc-500 dark:border-zinc-700/60 bg-white dark:bg-zinc-900/40 rounded-xl transition-all overflow-clip">
    <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-start">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-black dark:text-zinc-200 text-sm font-semibold">
          {subtitle}
        </p>
...
const IntegrationCard = ({
  ...
  >
    <div className="flex gap-2 items-center">
      <div
        className={`size-9 rounded-full flex justify-center items-center border shrink-0 ${
          connected
      ...
      >
        <Icon
          className={`size-5 ${connected ? "text-emerald-600 dark:text-emerald-400" : "text-black dark:text-zinc-400"}`}
        />
      </div>
      <div>
        <div className="flex items-center gap-1.5">
          <p className="text-[13px] text-black dark:text-zinc-100 font-semibold leading-tight">
            {title}
          </p>
          {connected && (
    ...
    <span
      className={`text-[11px] font-medium underline underline-offset-2 shrink-0 self-center ${
        connected
