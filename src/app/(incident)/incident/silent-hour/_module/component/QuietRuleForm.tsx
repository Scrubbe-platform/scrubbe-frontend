...
const QuietRuleForm = ({ onClose, onSave, editItem }: Props) => {
  ...
  const handleSave = () => {
    ...
    onSave?.({
      ...
      startTime,
      endTime,
      timezone: tzRef.current?.value ?? "UTC",
      daysOfWeek: [1, 2, 3, 4, 5],
      teamScope: teamRef.current?.value,
      isActive: true,
    });
    onClose();
  };
  ...
  return (
    <div>
      <Input
        ...
        label="Team / Service name"
        ref={nameRef as React.RefObject<HTMLInputElement>}
        defaultValue={editItem?.name}
      />
      <div className="space-y-1">
        <label className="text-sm text-white font-medium" htmlFor="">
          Quiet Window
        </label>
        <div className="grid grid-cols-2 items-center gap-3 w-full">
          <Input
            ...
            label="Start"
            type="time"
            ref={startRef as React.RefObject<HTMLInputElement>}
          ...
          <Input
            ...
            label="End"
            type="time"
            ref={endRef as React.RefObject<HTMLInputElement>}
      ...
      <Input
        labelClassName="text-white"
        className="text-white"
        label="Team Scope (optional)"
        placeholder="e.g. payments-team"
        ref={teamRef as React.RefObject<HTMLInputElement>}
        defaultValue={editItem?.teamScope}
      />

      <div className="flex justify-end gap-4">
        <CButton
          onClick={onClose}
          className="border border-IMSCyan bg-transparent hover:bg-transparent text-IMSCyan w-fit"
        >
          Cancel
        </CButton>
