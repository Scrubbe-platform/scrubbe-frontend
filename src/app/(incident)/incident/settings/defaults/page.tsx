...
const page = () => {
  ...
  const { control, handleSubmit, reset, watch, setValue } = useForm<DefaultsFormValues>({
    defaultValues: {
      defaultSeverity: 'P3',
            ...
            <section className="bg-transparent border border-neutral-500 rounded-[24px] p-4 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input className='bg-[#0B1224] text-white' placeholder='repo + pr + runId + sha' {...field} />
                  )}
                />
                <p className="text-[#64748B] text-xs leading-normal px-1">
                  Controls &quot;one incident vs spam.&quot; Use field names from your webhook payload.
                </p>
              </div>
...
const ToggleField = ({ label, desc, checked, onChange }: { label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }) => (
  <div className="flex justify-between items-center p-5 border border-neutral-500 rounded-2xl">
    ...
    <button
      ...
      className={`w-12 h-6 rounded-full relative transition-colors duration-200 ${checked ? 'bg-[#4ADE80]' : 'bg-neutral-500'}`}
    >
