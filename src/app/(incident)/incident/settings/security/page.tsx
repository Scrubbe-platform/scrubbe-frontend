...
const InfoPill = ({ icon, label }: { icon: React.ReactNode; label: string }) => (
  <div className="flex items-center gap-2 px-3 py-1.5 border border-zinc-500 dark:border-neutral-500 rounded-full bg-zinc-50 dark:bg-[#0B1224]/50">
    {icon}
    <span className="text-[11px] font-bold text-black dark:text-white uppercase tracking-wider">{label}</span>
  </div>
);
 
const ValuePill = ({ value }: { value: string }) => (
  <div className="px-4 py-1.5 border border-zinc-500 dark:border-neutral-500 rounded-full bg-zinc-50 dark:bg-[#0B1224]/50 text-[11px] font-mono text-zinc-600 dark:text-[#D1D5DB]">
    {value}
  </div>
);
  ...
  return (
    <div>
      <SettingWrapper title="Security" description="Identity, SSO, provisioning, and network controls" sub="This section is critical for enterprise onboarding.">
        <div className="grid 2xl:grid-cols-2 gap-8 items-start mb-8 pt-4">
          ...
          <section className="bg-transparent border border-zinc-500 dark:border-neutral-500 rounded-[24px] p-6 space-y-6">
            <div className="flex justify-between items-center px-1">
              <h3 className="text-black dark:text-white font-bold text-[17px]">Single Sign-On (SSO)</h3>
                ...
                <div className="rounded-2xl border border-[#F59E0B]/30 bg-zinc-50 dark:bg-[#0B1224]/70 px-4 py-4 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-black dark:text-[#D1D5DB]">This domain must be verified before SSO can be activated.</p>
                    </div>
                  ...
                  {verificationToken && (
                    <div className="rounded-xl border border-zinc-500 dark:border-white/10 bg-zinc-50 dark:bg-black/20 px-3 py-2">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-black dark:text-[#64748B]">Verification token</p>
            ...
            <div className="flex flex-wrap gap-3 px-1">
              <button onClick={() => runSsoTest()} disabled={isTesting} className="text-black dark:text-white border border-zinc-500 dark:border-white/15 px-4 py-2 rounded-xl text-xs font-bold bg-zinc-50 dark:bg-white/5 hover:bg-zinc-100 dark:hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
          ...
          {/* RIGHT — Network & API */}
          <section className="bg-transparent border border-zinc-500 dark:border-neutral-500 rounded-[24px] p-6 space-y-4">
            <h3 className="text-black dark:text-white font-bold text-[17px] px-1">Network & API keys</h3>
            ...
            {[
              ...
              <div key={item.label} className="p-5 border border-zinc-500 dark:border-neutral-500 rounded-2xl space-y-3 bg-white dark:bg-transparent">
                ...
                {item.label === "IP allowlist" && config?.ipAllowlist && config.ipAllowlist.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {config.ipAllowlist.map((ip, i) => (
                      <span key={i} className="text-[10px] font-mono px-2 py-0.5 border border-zinc-500 dark:border-white/10 rounded bg-zinc-50 dark:bg-white/5 text-zinc-600 dark:text-gray-300">{ip}</span>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="p-5 border border-zinc-500 dark:border-neutral-500 rounded-2xl flex justify-between items-center bg-white dark:bg-transparent">
        ...
        <section className="bg-transparent border border-zinc-500 dark:border-neutral-500 rounded-[24px] p-6 space-y-6">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-black dark:text-white font-bold text-[17px]">Provisioning (SCIM)</h3>
          ...
          <div className="p-4 border border-zinc-500 dark:border-neutral-500 rounded-2xl flex gap-4">
            {[
              ...
              <div key={item.label} className="flex items-center gap-2 px-4 py-1.5 border border-zinc-500 dark:border-neutral-500 rounded-full bg-zinc-50 dark:bg-[#0B1224]/50 text-[11px] text-zinc-600 dark:text-[#D1D5DB] font-bold uppercase tracking-widest">
                {item.icon}
                {item.label}
              </div>
            ))}
