      ...
      <SettingWrapper title='Security' description='Identity, SSO, provisioning, and network controls' sub='This section is critical for enterprise onboarding.'>
        <div className="grid grid-cols-2 gap-8 items-start mb-8 pt-4">
          ...
          <section className="bg-transparent border border-neutral-500 rounded-[24px] p-6 space-y-6">
            <div className="flex justify-between items-center px-1">
              ...
              <button
                onClick={() => toast.info('SSO configuration requires contacting your workspace admin or setting up via your identity provider.')}
            ...
            <div className="p-5 border border-neutral-500 rounded-2xl space-y-4">
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2 px-3 py-1.5 border border-neutral-500 rounded-full bg-[#0B1224]/50">
                  <ShieldCheck size={14} className={config?.ssoEnabled ? "text-[#4ADE80]" : "text-gray-500"} />
                  <span className="text-[11px] font-bold text-white uppercase tracking-wider">Status</span>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 border border-neutral-500 rounded-full bg-[#0B1224]/50">
                  <div className={`w-4 h-[2px] ${config?.ssoEnabled ? 'bg-[#4ADE80]' : 'bg-[#64748B]'}`} />
              ...
              <div className="flex justify-between items-center pt-2">
                <div className="flex items-center gap-2 px-3 py-1.5 border border-neutral-500 rounded-full bg-[#0B1224]/50">
                  ...
                  <span className="text-[11px] font-bold text-white uppercase tracking-wider">Enforce SSO</span>
                </div>
                <Switch
          ...
          <section className="bg-transparent border border-neutral-500 rounded-[24px] p-6 space-y-4">
            ...
            <div className="p-5 border border-neutral-500 rounded-2xl space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => toast.info('IP allowlist configuration coming soon.')}
              ...
              {config?.ipAllowlist && config.ipAllowlist.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {config.ipAllowlist.map((ip, i) => (
                    <span key={i} className="text-[10px] font-mono px-2 py-0.5 border border-white/10 rounded bg-white/5 text-gray-300">{ip}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="p-5 border border-neutral-500 rounded-2xl space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => toast.info('Manage API keys in the Integrations section.')}
                  className="text-[#00CAD8] border border-[#00CAD8] px-3 py-1 rounded-lg text-[10px] font-bold hover:bg-[#00CAD8]/5"
                >
                  Manage
                </button>
        ...
        <section className="bg-transparent border border-neutral-500 rounded-[24px] p-6 space-y-6">
          <div className="flex justify-between items-center px-1">
            ...
            <button
              onClick={() => toast.info('SCIM provisioning configuration requires an enterprise plan. Contact support.')}
              className="text-[#00CAD8] border border-[#00CAD8] px-4 py-1.5 rounded-xl text-xs font-bold hover:bg-[#00CAD8]/5 transition-all"
            >
              Configure
            </button>
        ...
        <div className="pt-6 flex justify-end">
          <CButton className='w-fit px-4' onClick={handleSave} isLoading={isPending} disabled={isPending}>
            Save
          </CButton>
        </div>
      </SettingWrapper>
