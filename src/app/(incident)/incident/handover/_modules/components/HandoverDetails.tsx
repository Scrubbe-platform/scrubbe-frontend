...
export default function HandoverDetailPage() {
  ...
  useEffect(() => {
    ...
    const update = () =>
      setSecondsLeft(Math.round((deadline - Date.now()) / 1000));
    update();
  ...
  const handleCatchMeUp = async () => {
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to generate Ezra brief",
      );
    }
  };
  ...
  const handleRemoveIncident = async (linkId: string) => {
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to remove incident",
      );
    }
  };
  ...
  const severity =
    primaryLink?.incident.severity || primaryLink?.incident.priority || "—";
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() =>
                setIsOpenModal((prev) => ({
                  ...prev,
                  attachHandover: !prev.attachHandover,
                }))
              }
            >
              Attach to Handover
            ...
            <Button
              ...
              onClick={() =>
                setIsOpenModal((prev) => ({
                  ...prev,
                  transferOwnership: !prev.transferOwnership,
                }))
              }
            >
              Transfer Ownership
        ...
        <Card className="p-6 mb-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Pill className="text-zinc-600 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700">
              Severity{" "}
              <span className="text-red-500 font-bold ml-1">{severity}</span>
            </Pill>
            <Pill className="text-zinc-600 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700">
              Created{" "}
        ...
        >
          <div>
            ...
            <p className="text-[12px] text-zinc-500">
              Commander:{" "}
              {handover.currentCommander
                ? `${handover.currentCommander.firstName} ${handover.currentCommander.lastName}`
                : "Unassigned"}
            </p>
          </div>
          <div className="text-center">
            <p className="text-[20px] font-black text-emerald-400 mb-1">
              {new Date(handover.transferAt).toLocaleTimeString("en-US", {
                hour: "2-digit",
                minute: "2-digit",
                timeZone: "UTC",
              })}{" "}
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  onClick={() =>
                    setIsOpenModal((prev) => ({
                      ...prev,
                      addIncidentHandover: !prev.addIncidentHandover,
                    }))
                  }
            ...
            {handover.incidents.map((link) => (
              <div key={link.id} className="px-5 py-4">
                <div
                  ...
                  onClick={() => toggleIncident(link.id)}
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <Pill className="flex items-center gap-1.5 text-zinc-600 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700">
                      <StatusDot
                        color={severityColor(link.incident.severity)}
                      />{" "}
                      {link.incident.status}
                    </Pill>
                    <span className="text-[12px] font-bold text-amber-500">
                      {link.incident.severity || link.incident.priority}
                    </span>
                ...
                {expanded[link.id] && (
                  <div className="ml-6 mt-2">
                    ...
                    <Link
                      href={`/incident/tickets?id=${link.incident.id}`}
            ...
            >
              ...
              {handover.ezraBrief ? (
                <div className="text-left max-w-2xl mx-auto">
                  <p className="text-[15px] text-zinc-100 leading-relaxed mb-4">
                    {handover.ezraBrief.executiveSummary}
                  </p>
                    ...
                    {handover.ezraBrief.rcaConfidence !== undefined && (
                      <span>
                        ...
                        <strong className="text-emerald-400">
                          {Math.round(
                            (handover.ezraBrief.rcaConfidence ?? 0) * 100,
                          )}
                          %
                        </strong>
                      </span>
                    )}
                    {handover.ezraBrief.derivedSeverity && (
                      <span>
                        Severity: {handover.ezraBrief.derivedSeverity}
                      </span>
                    )}
                    {handover.ezraBrief.customerExposure && (
                      <span>
                        Exposure: {handover.ezraBrief.customerExposure}
                      </span>
                    )}
                  </div>
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {[
                  ...
                  },
                  {
                    label: "Risk Score",
                    value: incidentDetail
                      ? `${Math.round(incidentDetail.riskScore)}`
                      : "—",
                  },
                  {
                    label: "Blast Radius",
            ...
            <Card className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    setIsOpenModal((prev) => ({
                      ...prev,
                      addHandoverNote: !prev.addHandoverNote,
                    }))
                  }
              ...
              <div className="space-y-4">
                {(handover.notes ?? []).map((note, i) => (
                  <div
                    key={note.id}
                    className={`flex gap-3 ${i < (handover.notes?.length ?? 0) - 1 ? "pb-4 border-b border-zinc-100 dark:border-zinc-800" : ""}`}
                  >
      ...
      <AttachExistingHandover
        ...
        onClose={() =>
          setIsOpenModal((prev) => ({ ...prev, attachHandover: false }))
        }
      ...
      <TransferOwnershipModal
        ...
        onClose={() =>
          setIsOpenModal((prev) => ({ ...prev, transferOwnership: false }))
        }
        handover={handover}
      ...
      <AddIncidentModal
        ...
        onClose={() =>
          setIsOpenModal((prev) => ({ ...prev, addIncidentHandover: false }))
        }
      ...
      <AddNoteModal
        ...
        onClose={() =>
          setIsOpenModal((prev) => ({ ...prev, addHandoverNote: false }))
        }
        handoverId={handover.id}
        onAdded={invalidate}
      />
