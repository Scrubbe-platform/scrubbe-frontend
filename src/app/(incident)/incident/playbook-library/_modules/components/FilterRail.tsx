...
export default function FilterRail({
  ...
  return (
    <div className="bg-white shadow-sm shadow-light rounded-xl sticky top-20 max-h-[calc(100vh-120px)] overflow-y-auto dark:bg-zinc-900/40">
        ...
        {FILTER_DEFS.map((def) => {
          ...
          return (
            <details key={def.key} open className="group">
              <summary className="flex items-center justify-between px-4 py-2.5 cursor-pointer list-none text-xs font-medium text-zinc-600 select-none dark:text-zinc-400">
                ...
                <ChevronDown
                  size={13}
                  className="text-zinc-400 group-open:rotate-180 transition-transform dark:text-zinc-500"
                />
              </summary>
              <div className="px-3 pb-3 flex flex-col gap-0.5">
                {def.opts.map((opt) => {
                  ...
                  return (
                    <label
                      key={opt}
                      className="flex items-center gap-2 px-1.5 py-1 rounded cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800"
                    >
                      <input
                        type="checkbox"
                        checked={isChecked}
                      ...
                      />
                      <span className="flex-1 text-xs text-zinc-700 truncate dark:text-zinc-400">{opt}</span>
                      <span
                        className={`text-[10px] tabular-nums ${
                          count > 0 ? "text-zinc-500 font-medium dark:text-zinc-500" : "text-zinc-300 dark:text-zinc-600"
                        }`}
                      >
                        {count}
                      </span>
