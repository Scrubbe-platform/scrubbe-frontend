    ...
    >
      <div className="grid gap-3 mb-4" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(215px,1fr))" }}>
        <div className="border border-zinc-200 rounded-lg bg-zinc-50 px-3.5 py-3">
          <div className="text-xs text-zinc-500 mb-1.5">Apply mode</div>
          <button
            onClick={() => {
        ...
        <div className="border border-zinc-200 rounded-lg bg-zinc-50 px-3.5 py-3 sm:col-span-2">
      ...
      <label className="block text-xs font-semibold text-zinc-600 mb-2">Match rules</label>
      <div className="flex gap-1.5 flex-wrap mb-3">
        ...
        {scopeRules.map((r, i) => (
          <span
            key={`${r}-${i}`}
            className="inline-flex items-center gap-1.5 border border-zinc-300 bg-white rounded-full pl-3 pr-1.5 py-1 font-mono text-xs"
          >
            {r}
            <button
              onClick={() => {
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 min-w-[180px] border border-zinc-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-500"
        />
        <button
          onClick={() => {
      ...
      <label className="block text-xs font-semibold text-zinc-600 mt-5 mb-2">
        Matched resources <span className="text-zinc-400 font-normal">({matched.length})</span>
      </label>
      <div className="flex gap-1.5 flex-wrap mb-4">
        {matched.map((r) => (
          <span key={r} className="border border-zinc-200 bg-zinc-50 rounded-full px-3 py-1 text-xs">
            {r}
          </span>
        ))}
      </div>
      <button
        ...
        className="px-3.5 py-2 text-sm font-semibold border border-zinc-300 rounded-lg hover:bg-zinc-50 disabled:opacity-50 transition-colors"
      >
