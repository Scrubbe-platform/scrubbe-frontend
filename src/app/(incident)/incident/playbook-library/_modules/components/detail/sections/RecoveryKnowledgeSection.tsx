    ...
    <>
      ...
      >
        <div className="grid gap-2.5" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))" }}>
          {RECOVERY.map((c) => (
            <div key={c.name} className="flex items-center gap-3 border border-zinc-200 rounded-lg px-3.5 py-3">
              <span className="font-semibold text-[13px]">{c.name}</span>
              {validated ? (
                <span className="ml-auto inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600">
                  <Check size={11} /> {c.state}
                </span>
              ) : (
                <span className="ml-auto text-xs font-semibold px-2.5 py-1 rounded-full bg-zinc-100 text-zinc-500">
                  Awaiting run
                </span>
              )}
            </div>
        ...
        <div className="grid gap-3.5 mt-4" style={{ gridTemplateColumns: "1fr auto 1fr" }}>
          <div className={`border rounded-lg px-4 py-3.5 ${validated ? "border-emerald-400 bg-emerald-50" : "border-zinc-200"}`}>
            <h5 className="font-bold text-[13px]">All checks pass</h5>
            <p className={`text-xs mt-1 ${validated ? "text-emerald-700" : "text-zinc-500"}`}>
              Incident moves to Monitoring automatically.
            </p>
          </div>
          <span className="self-center font-mono text-[11px] text-zinc-400">OTHERWISE</span>
          <div className="border border-zinc-200 rounded-lg px-4 py-3.5">
            <h5 className="font-bold text-[13px]">Any check fails</h5>
            <p className="text-xs text-zinc-500 mt-1">Automation pauses and the incident continues investigation.</p>
          </div>
        </div>
